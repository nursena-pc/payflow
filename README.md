...
OpenAPI documentation covers the implemented API. Across the committed standard, MFA, login-rate-limit, and compatibility-coverage collections, Postman represents all 30 canonical `/api/v1` operations while keeping lifecycle-sensitive and privileged requests manually gated. PayFlow v1.0.0 is the latest tagged and published release at exact Maven/OpenAPI version `1.0.0`. Release hardening and immutable publication are complete; publication-record and release-train closure is tracked by [issue #208](https://github.com/nursena-pc/payflow/issues/208). Registration remains evidence-backed `DEFER`, and the existing password-login limiter semantics remain unchanged.

The immutable v1.0.0 publication record is anchored to annotated tag `v1.0.0` with tag object `bc1750b17bbdfdcc24764c943afbfd9943e33190`, merge/tag target `1adf0b38f1d2da82097c58e41aea4e36a2b4a643`, and successful Release workflow run [33388085847](https://github.com/nursena-pc/payflow/actions/runs/33388085847). GitHub Release ID `379712093` was published at `2026-08-31T11:50:28Z`. The published `payflow-1.0.0.jar` is 100566872 bytes and its independently verified SHA-256 is `ed58f5e812e6dfd3ee1ced8f480265bbc6221ff5de760253095762294a9dd8b1`; checksum asset SHA-256 `24d66af06c41b1c5bad37c218c8a20678b7fe85d7cfb717ae28a8916cfd79a5c` names and matches that JAR, and the published release notes exactly match the reviewed versioned notes.
...
The v1 line began from exact v0.16.0 publication-record merge `7712c5ccbeeee3b9cefd3324c42270e71554ea17` and completed release hardening through development-start #190, authentication/security lifecycle closure #192, financial/messaging integrity closure #194, observability/performance closure #197, recovery/migration/API/documentation freeze #199, supply-chain/clean-environment verification #201, release preparation #203, and immutable publication #207. Publication-record and release-train closure is tracked by [issue #208](https://github.com/nursena-pc/payflow/issues/208).

PayFlow v1.0.0 is published at exact Maven and OpenAPI version `1.0.0`. The public `/api/v1` surface remains frozen at 30 canonical operations across 28 normalized paths. The immutable publication is anchored to the annotated `v1.0.0` tag and the exact approved release merge; the publication record does not mutate or recreate the tag or GitHub Release.

The v1 release re-validates authentication/security lifecycle behavior, transaction/ledger/outbox/Kafka/DLQ guarantees, observability and bounded performance evidence, PostgreSQL backup/restore and Flyway migration rehearsals, Redis/Kafka recovery procedures, `/api/v1` compatibility, OpenAPI/Postman/documentation alignment, supply-chain evidence, and clean-environment release verification. Existing modular-monolith, simulated-money, registration `DEFER`, password-login limiter, fail-closed, privacy, and credential-redaction boundaries remain in force.

CP4 produced bounded developer-workstation performance evidence; CP5 froze recovery, migration, API, OpenAPI, Postman, and documentation contracts; CP6 re-validated committed-content secret scanning, SBOM/vulnerability evidence, isolated Maven-cache clean verification, required-configuration fail-fast behavior, and production-profile Docker smoke; CP7 froze the exact release candidate; and CP8 immutably published and independently verified v1.0.0. These records do not create production-capacity, regulatory, real-money, signing, SLSA, provenance-attestation, or reproducible-build claims.
## Implemented API
