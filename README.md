...
OpenAPI documentation and the executable Postman collection cover the implemented API. Releases through `v0.4.0` are tagged, and the current delivery focus is preparing `v0.5.0`, which introduces the secure operations control plane. See the [roadmap](docs/roadmap.md).
...
Run the **Operations** folder separately. It is intentionally manual and requires a valid admin JWT in `operatorAccessToken` plus an existing dead-letter UUID in `deadLetterRecordId`. Neither value is generated by the standard workflow or committed to the repository.
